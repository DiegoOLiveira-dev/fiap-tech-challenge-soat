...
export class PaymentPersistenceAdapter implements PaymentPersistencePort {
  ...
  async persistPayment(payment: Payment): Promise<Payment> {
    try {
  ...
  async updateOrderById(id_order, status_payment): Promise<Payment> {
    const status = await this.dataServices.payment.getByFilter({
      _id: id_order,
    });
    if (status[0].status_payment == "true") {
      ...
      const allPayments = await this.dataServices.payment.updateById(
        id_order,
        status[0]
      );

      await this.dataServices.pedido.patchById(status[0].id_order, {
        date: new Date().toString(),
        status: "65b02ea665520ae647add25e",
      });
